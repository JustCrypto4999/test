...
newMenu( menu, access = 0 )
{
    if( access >= self.access )
        return self iPrintLnBold( "Access level denined." );
    if(!isDefined( menu ))
    {
    ...
    if(menu == "clientOpts" && self getCursor() > level.players.size - 1)
        self setCursor(0);
...
updateSlider(pressed, curs = self getCursor(), rcurs = self getCursor())
{
    ...
    if (IsDefined(self.eMenu[rcurs].ID_list))
    {
        ...
        oldValue = value; // Save the old value to check for changes
        ...
        count = " [" + (value + 1) + "/" + (self.eMenu[rcurs].ID_list.size) + "]";
        self.menu["UI_SLIDE"]["STRING_" + cap_curs] setText(self.eMenu[rcurs].RL_list[value] + count);

        // Play the scroll sound only if the value has changed
        if (value != oldValue)
        {
    ...
    self.sliders[self getCurrentMenu() + "_" + rcurs] = self.eMenu[rcurs].val;
        ...
        oldValue = self.sliders[self getCurrentMenu() + "_" + rcurs]; // Save the old value to check for changes
...
setCursor(value, menu = self getCurrentMenu())
{
    if(!isDefined(value))
    {
        value = self.menu[menu + "_cursor"] - 1;
        if(value < 1)
            value = 0;
    }
