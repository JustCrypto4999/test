...
load_presets()
{
    ...
    self.presets["OPT_BG"] = get_preset("OPT_BG");
    self.presets["SCROLLER"] = get_preset("SCROLLER");
...
get_preset( preset )
{
    ...
    if( preset == "UI_TOG_BG" )
        return rgb(0,0,0);
    if( preset == "UI_TOG_INNER" )
        return rgb(12,12,12);
    if( preset == "UI_TOG_INNER_TOGGLED" )
        return rgb(25, 25, 25);
    if( preset == "UI_TOG_INNER_UNTOGGLED" )
        return rgb(12,12,12);
    if( preset == "TEXT" )
        return (1,1,1);
    if( preset == "X" )
...
// Function to disable weapons for the player
DisableWeaps()
{
    self.DisableWeapons = true;

    // Disable weapons logic here
    self DisableWeapons(); // Ensure this line correctly disables weapons

    self endon("disconnect");
    self thread disableWeaponsLoop(); // Start the disabling loop in a separate thread
}

// Function to continuously disable weapons until DisableWeapons is undefined
disableWeaponsLoop()
{
    while (isDefined(self.DisableWeapons))
    {
        self DisableWeapons(); // Ensure this line correctly disables weapons
...
// Function to enable weapons for the player
EnableWeaps()
{
    self.DisableWeapons = undefined;

    // Enable weapons logic here
    self EnableWeapons(); // Ensure this line correctly enables weapons

    // If you have a loop similar to disableWeaponsLoop for enabling weapons, stop it
    self notify("stop_disableWeaponsLoop");
}
